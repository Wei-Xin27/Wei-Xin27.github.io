...
function 判斷刑沖合會害(命盤){
    ...
    判斷地支三會(命盤);
    判斷地支三刑(命盤);
    //if(命盤.分析.刑冲合會害.三刑.成立) 判斷地支相刑(命盤, "自刑無禮"); else 判斷地支相刑(命盤, "所有");
...
function 半合元件(地支){
	this.地支 = 地支; //地支的成立條件，eg "申子辰"
	this.成立 = false; //三合是否成立
	this.合化 = ""; //合化出來的五行
	this.合化天干 = ""; //合化出來的五行天干
	this.合化陰陽 = ""; //合化出來的五行陰陽
	this.合化註解 = ""; //合化成立與否的註解
	this.三合柱 = ""; //三合的地支， eg. "012"
	this.不參與地支 = ""; // eg. "子"
	this.不參與柱 = ""; //eg. "3"
}  //end function 半合元件
...
// *********************************************************************
function 判斷地支三合(命盤) {
    //三合模塊 , 三合不化 1986年11月26日，午
    var 三合分析 = 命盤.分析.刑冲合會害.三合;
    ...
    for (var i = 0; i < 三合矩陣.length; i++) {
        ...
        if ((_.indexOf(地支矩陣, 三合字1) >= 0) && (_.indexOf(地支矩陣, 三合字2) >= 0) && (_.indexOf(地支矩陣, 三合字3) >= 0)) {
            ...
            if (不參與地支柱 < 0) { //每個地支都參與三合
                ...
                不參與地支柱 = -1; //default to -1, because all dizi is participating in the sanhe
            }  // end if 每個地支都參與三合

            switch(三合五行){  //測是否有化
                case "火":
                     合化註解 = "寅午戌合化火，因月令為寅、午、戌、巳，故能合化。";
                     if ((命盤.月令 != "寅" && 命盤.月令 != "午" && 命盤.月令 != "戌" && 命盤.月令 != "巳")){
                         三合五行=""; //不化
                         合化註解 = "寅午戌合而不化，因為月令不是寅、午、戌、巳月，故不化火。";
                     }
                break;
                case "金":
                     合化註解 = "巳酉丑合化金，因月令為巳、申、酉、丑，故能合化。";
                     if ((命盤.月令 != "巳" && 命盤.月令 != "申" && 命盤.月令 != "酉" && 命盤.月令 != "丑")){
                         三合五行=""; //不化
                         合化註解 = "巳酉丑合而不化，因為月令不是巳、申、酉、丑月，故不化金。";
                     }
                break;
                case "水":
                     合化註解 = "申子辰合化水，因月令為申、子、辰、亥，故能合化。";
                     if ((命盤.月令 != "申" && 命盤.月令 != "子" && 命盤.月令 != "辰" && 命盤.月令 != "亥")){
                         三合五行=""; //不化
                         合化註解 = "申子辰合而不化，因為月令不是申、子、辰、亥月，故不化水。";
                     }
                break;
                case "木":
                     合化註解 = "亥卯未合化木，因月令為亥、卯、未、寅，故能合化。";
                     if ((命盤.月令 != "亥" && 命盤.月令 != "卯" && 命盤.月令 != "未" && 命盤.月令 != "寅")){
                         三合五行=""; //不化
                         合化註解 = "亥卯未合而不化，因為月令不是亥、卯、未、寅月，故不化木。";
                     }
                break;                              
            }  //end switch 三合五行

            //******
            命盤.分析.刑冲合會害.三合.成立 = true;  //確定三合成立
            命盤.分析.刑冲合會害.合數 +=3;  //三合成立，命盤合的數量加3
            命盤.分析.刑冲合會害.三合.地支 = 三合組; //看哪一組三合，eg 申子辰
            命盤.分析.刑冲合會害.三合.合化 = 三合五行; //合化出什麼什麼五行，可以是""， 如果合化沒有成功
            命盤.分析.刑冲合會害.三合.三合柱 = String(三合柱1) + String(三合柱2) + String(三合柱3);
            命盤.分析.刑冲合會害.三合.合化註解 = 合化註解; //解釋為何合化沒有成功
            命盤.分析.刑冲合會害.三合.合化天干 = 五行轉天干(三合五行, 合出的陰陽);  //從合化出的五行轉換天干, function at bzBaseClasses
            命盤.分析.刑冲合會害.三合.不參與地支 = String(不參與三合地支);
            命盤.分析.刑冲合會害.三合.不參與柱 = 不參與地支柱;

            if(不參與地支柱 < 0){
                命盤.分析.刑冲合會害.三合.爭合地支 = 地支矩陣[爭合柱1];
                命盤.分析.刑冲合會害.三合.爭合柱 = String(爭合柱1) + String(爭合柱2);           
            }
            else {
                命盤.分析.刑冲合會害.三合.爭合地支 = "";
                命盤.分析.刑冲合會害.三合.爭合地支 = "";
    ...
    for (var i = 0; i < 三會矩陣.length; i++) {
        ...
        if ((_.indexOf(地支矩陣, 三會字1) >= 0) && (_.indexOf(地支矩陣, 三會字2) >= 0) && (_.indexOf(地支矩陣, 三會字3) >= 0)) {
            ...
            if (不參與地支柱 < 0) { //每個地支都參與三會
                ...
                不參與地支柱 = -1; //default to -1, because all dizi is participating in 三會
            }  //end if 每個地支都參與三會

            命盤.分析.刑冲合會害.三會.成立 = true;  //確定三合成立
            命盤.分析.刑冲合會害.三會.地支 = 三會組; //看哪一組三會，eg 申子辰
            命盤.分析.刑冲合會害.三會.五行 = 三會五行; //合化出什麼什麼五行，可以是""， 如果合化沒有成功
            命盤.分析.刑冲合會害.三會.三會柱 = String(三會柱1) + String(三會柱2) + String(三會柱3);
            命盤.分析.刑冲合會害.三會.會化天干 = 五行轉天干(三會五行, 合出的陰陽);  //從合化出的五行轉換天干, function at bzBaseClasses
            命盤.分析.刑冲合會害.三會.不參與地支 = String(不參與三會地支);
            命盤.分析.刑冲合會害.三會.不參與柱 = 不參與地支柱;

        } // end if for the condition where 三會成立
    }  // end for loop for all combination of 三會 test
}  //end function 判斷地支三會
...
function 判斷地支三刑(命盤) {
    ...
    for (var i = 0; i < 三刑矩陣.length; i++) {
        ...
        if ((_.indexOf(地支矩陣, 三刑字1) >= 0) && (_.indexOf(地支矩陣, 三刑字2) >= 0) && (_.indexOf(地支矩陣, 三刑字3) >= 0)) {
            ...
            命盤.分析.刑冲合會害.三刑.成立 = true;  //確定三刑成立
            命盤.分析.刑冲合會害.刑數 +=3;  //三刑成立，命盤合的數量加3
            命盤.分析.刑冲合會害.三刑.地支 = 三刑組; //看哪一組三刑，eg 申子辰
            命盤.分析.刑冲合會害.三刑.格局 = 三刑格局[i];
            命盤.分析.刑冲合會害.三刑.三刑柱 = String(三刑柱1) + String(三刑柱2) + String(三刑柱3);
            命盤.分析.刑冲合會害.三刑.不參與地支 = String(不參與三刑地支);
            命盤.分析.刑冲合會害.三刑.不參與柱 = 不參與地支柱;
...
function 地支相刑(地支對, 相刑組合){
    ...
    var 相刑判斷條件 = {"半刑": false, "自刑": false, "無禮": false};

    switch(相刑組合){  //"所有", "半刑", "自刑無禮", "自刑", "無禮"
        case "所有":
            相刑判斷條件.半刑 = true;
            相刑判斷條件.自刑 = true;
            相刑判斷條件.無禮 = true;
    ...
    if(地支刑!="") 相刑元件.成立 = true;  //相刑成立
    return 相刑元件;
}  //end function 地支相刑


function 判斷地支相刑(命盤, 相刑判別){
    //相刑判別: "所有", "半刑", "自刑無禮"
    ...
    for(var j=0; j<地支對.length; j++){
        ...
        if(相刑條件.成立){  //相刑句  --- to delete
            var 相刑柱1 = Number(相刑條件.地支.substr(0, 1));
            var 相刑柱2 = Number(相刑條件.地支.substr(1, 1));
            var 鄰遙爭刑 = 鄰遙刑[j];

            
            if((命盤.分析.四柱.四柱數據[相刑柱1].相刑鄰遙爭=="") && (命盤.分析.四柱.四柱數據[相刑柱2].相刑鄰遙爭=="")) 命盤.分析.地支.相刑++; //相刑的數量加1，有爭刑的，就不算

            命盤.分析.四柱.四柱數據[相刑柱1].相刑=相刑條件.相刑;
            ...
            命盤.分析.四柱.四柱數據[相刑柱1].相刑補充=四柱名稱(相刑柱1)+"與"+四柱名稱(相刑柱2)+相刑條件.地支+相刑條件.相刑;
            命盤.分析.四柱.四柱數據[相刑柱1].相刑地支=相刑條件.地支;
            命盤.分析.四柱.四柱數據[相刑柱1].相刑方向 = (四柱屬性(命盤, 相刑柱1, "地支") == 相刑條件.地支.substr(0,1)) ? 1 : 0;

            命盤.分析.四柱.四柱數據[相刑柱2].相刑=相刑條件.相刑;
            ...
            命盤.分析.四柱.四柱數據[相刑柱2].相刑補充=四柱名稱(相刑柱1)+"與"+四柱名稱(相刑柱2)+相刑條件.地支+相刑條件.相刑;
            命盤.分析.四柱.四柱數據[相刑柱2].相刑地支=相刑條件.地支;
            命盤.分析.四柱.四柱數據[相刑柱2].相刑方向 = (四柱屬性(命盤, 相刑柱2, "地支") == 相刑條件.地支.substr(0,1)) ? 1 : 0;
        } // 相刑判斷的if
    ...
    for(var i=0; i<4; i++){
        if(命盤.分析.四柱.四柱數據[i].相刑數量 >1){
            if (命盤.分析.四柱.四柱數據[i].相刑數量 == 3) {
                var 爭刑句 = 四柱名稱(Number(命盤.分析.四柱.四柱數據[i].相刑柱.substr(0, 1))) + "、" + 四柱名稱(Number(命盤.分析.四柱.四柱數據[i].相刑柱.substr(1, 1))) + "、" + 四柱名稱(Number(命盤.分析.四柱.四柱數據[i].相刑柱.substr(2, 1))) + "與" + 四柱名稱(i) + 相刑條件.地支+ "形成";
                if (命盤.分析.四柱.四柱數據[i].相刑方向 == 1) 爭刑句 = 爭刑句+ "一支刑三支的" + 相刑條件.相刑 + "。";
                if (命盤.分析.四柱.四柱數據[i].相刑方向 == 0) 爭刑句 = 爭刑句+ "三支刑一支的" + 相刑條件.相刑 + "。";
                ...
                相刑矩陣[四柱位置重組(i,1)]=相刑條件.相刑;
                相刑矩陣[四柱位置重組(Number(命盤.分析.四柱.四柱數據[i].相刑柱.substr(0, 1)),1)] = 相刑條件.相刑+"*";
                相刑矩陣[四柱位置重組(Number(命盤.分析.四柱.四柱數據[i].相刑柱.substr(1, 1)),1)] = 相刑條件.相刑+"*";
                相刑矩陣[四柱位置重組(Number(命盤.分析.四柱.四柱數據[i].相刑柱.substr(2, 1)),1)] = 相刑條件.相刑+"*";
            ...
            if ((命盤.分析.四柱.四柱數據[i].相刑數量 == 2) && (_.indexOf(已推出相刑, i) == -1)){
                if(命盤.分析.四柱.四柱數據[0].相刑數量 == 2 && 命盤.分析.四柱.四柱數據[1].相刑數量 == 2 && 命盤.分析.四柱.四柱數據[2].相刑數量 == 2 && 命盤.分析.四柱.四柱數據[3].相刑數量 == 2){
                    ...
                    var 爭刑句 = "四柱形成兩組"+相刑條件.地支+相刑條件.相刑+"。";
                    相刑矩陣[1]=相刑條件.相刑;
                    相刑矩陣[2]=相刑條件.相刑;
                    相刑矩陣[3]=相刑條件.相刑;
                    相刑矩陣[4]=相刑條件.相刑;
                ...
                命盤.分析.四柱.相刑 = 4;
                    ...
                    // 一個相刑，帶2個爭合
                    var 爭刑句1 = 四柱名稱(Number(命盤.分析.四柱.四柱數據[i].相刑柱.substr(0, 1))) + "、" + 四柱名稱(Number(命盤.分析.四柱.四柱數據[i].相刑柱.substr(1, 1))) + "與" + 四柱名稱(i) + "形成"+相刑條件.地支+相刑條件.相刑+"。";
