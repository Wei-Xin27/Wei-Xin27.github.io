...
function 西曆轉換(輸入西曆日期) {
    ...
    **/
    ...
    //$("#debuginfo").html("距離立春: " +距離立春日 + ", "+ 距離立春日2);
    ...
    if(輸入西曆日期 < 日期值轉換(曆法數據[YearData].小寒, 1)) {
        ...
        距離節的天數 = momentDateDiff(節日期, 輸入西曆日期);
    ...
    // ************************ 農曆、陰曆 計算模塊 **************************************
        function 農曆計算(計算的西曆,計算年份數據){
            ...
            距離初一天數 = momentDateDiff(正月初一日期, 計算的西曆);
            閏月字串 = "";
            算出月 = 1; //正月
            算出日 = 1; //初一
            DayCounter = 0;
    ...
    var 農曆日期 = 農曆年 + "（"+ 六十干支屬性(干支年,1) + "）年" + 農曆閏月字 + 農曆月 + "月" + 農曆日 + "日" + 時支 + "時";
    ...
    var 陰曆日期 = 六十干支屬性(陰曆干支年,1)+"年" + 陰曆閏月字 + 陰曆月 + "月" + 陰曆日 + "日" + 陰曆時支 + "時";
    var 陰曆數據 = [];
