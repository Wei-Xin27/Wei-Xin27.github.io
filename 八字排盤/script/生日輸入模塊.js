...
function 填寫時間(elementId, 是否填寫吉時) {
...
function Gen_生日輸入表格(ButtonStr, ButtonName){
    ...
    */
    ...
    生日輸入.push('曆法：<input name="mp-輸入曆法" type="radio" value="西曆" id="mp-輸性曆法西曆" checked>西曆&nbsp&nbsp&nbsp&nbsp');

    生日輸入.push('<input name="mp-輸入曆法" type="radio" value="農曆" id="mp-輸性曆法農曆">農曆&nbsp&nbsp&nbsp&nbsp');
    生日輸入.push('<input name="mp-輸入曆法" type="radio" value="干支曆" id="mp-輸性曆法干支曆"><span id="mp-輸性干支曆標題">干支曆&nbsp&nbsp&nbsp&nbsp</span>');
    ...
    生日輸入.push('<div class="col-md-12" style="height: auto; padding-top: 5px; padding-bottom: 5px;">');
    生日輸入.push('日期：<select id="mp-輸入年" style="width:125px;"></select>&nbsp年&nbsp');
    生日輸入.push('<select id="mp-輸入月" style="width:50px;"></select>&nbsp月&nbsp');
    生日輸入.push('<span id="mp-chk-disp-輸入月-閏月"><input type="checkbox" id="mp-輸入月-閏月" value=1>閏月&nbsp</span>');
    生日輸入.push('<select id="mp-輸入日" style="width:50px;"></select>&nbsp日&nbsp');
    生日輸入.push('<select id="mp-輸入時" style="width:110px;"></select>&nbsp時&nbsp');
    生日輸入.push('<select id="mp-輸入分" style="width:50px;"></select>&nbsp分 ');
    生日輸入.push('</div>');
    生日輸入.push('</div>');
...
function 初始化生日輸入(姓名輸入, 性別輸入, 農曆輸入, 干支曆輸入, 其他選項輸入, 吉時選項) {
...
function 建立八字表格(){
    ...
    BaziChart.push('<td colspan="2" rowspan="3" class="bzb-命盤複合格"><img class="displayed" alt="Logo" height="96px" width="89px" style="margin: 0px 0px 0px 0px" src=' + 系統.版本.BodyImg+'></td>');
    BaziChart.push('</tr>');
    ...
    BaziChart.push('<td colspan="2" rowspan="3" class="bzb-命盤複合格"><img class="displayed" alt="Logo" height="92px" width="89px" style="padding: 0px 0px 0px 0px" src=' + 系統.版本.CopyrightImg+'></td>');
    BaziChart.push('</tr>');
    ...
    BaziChart.push('<td colspan="2" class="bzb-命盤單格 bzb-命盤中中黑色字型"><img class="displayed" alt="Logo" height="50px" width="89px" style="margin: 0px 0px 0px 0px" src=' + 系統.版本.XiaoYunImg+'></td>');
    BaziChart.push('</tr>');
...
*/
    ...
    滴天髓十干矩陣.push('<li data-author="乙為枝葉之木，柔如花卉，然坐丑未能制之，（丑未陰土，故乙能制）如宰羊割牛，只要有一丙丁，則雖申酉之月，亦不畏怯，生於子月，（木葉凋零之時，水多益寒）而又辛壬癸透者，則雖得午，亦難發生，（乙雖生午，然午能洩乙，況一火不能敵眾水也）若甲與寅多見，譬之縢蘿附喬木，春月秋月皆可。">乙 木 雖 柔， 刲 羊 解 牛， 懷 丁 抱 丙， 跨 鳳 乘 猴， 虛 濕 之 地， 騎 馬 亦 憂， 藤 蘿 繫 甲， 可 春 可 秋。</li>');
